...
export class ESPLoader {
  ...
  constructor(options: LoaderOptions) {
    ...
    // connected through native USB (USB-Serial-JTAG / USB-OTG): larger
    // blocks overflow the flasher stub's USB buffer and the stub answers
    // 0xC9 "Too much data" mid-write. Native-USB Espressif links are
    // recognizable by the Web Serial USB vendor id.
  ...
  async command(
    ...
    if (op != null) {
      // Discard stale input first: leftover packets would otherwise be
      // read as this command's response, desyncing the protocol.
      this.transport.flushInput();
  ...
  async _updateImageFlashParams(
    ...
    const shaAppended = this.chip.CHIP_NAME !== "ESP8266" && image[8 + 15] === 0x31; // '1' character code
    ...
    if (shaAppended) {
      ...
      // Combine all parts
      const finalImage = new Uint8Array(
        imageDataBeforeSha.length + shaDigestCalculatedUintArray.length + imageDataAfterSha.length,
      );
      ...
      const imageStoredSha = finalImage.slice(
